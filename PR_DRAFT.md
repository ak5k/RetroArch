...
CLI `-r` handler had no-op (`if (enable) enable = true`) that prevented `rec_st->enable` from being set, breaking async readback init when `video_gpu_record = true` is set in config.
